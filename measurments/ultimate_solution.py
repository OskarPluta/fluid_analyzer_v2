...
#     # def histogram(self, frame: np.ndarray):
#     #     """
#     #     Create a histogram of the frame to visualize the distribution of pixel values.
#     #     """
...
#     #     roi = gray_frame[self.upper_line:self.lower_line, 0:self.size[0]]
#     #     cv.imshow("ROI", roi)
...
#     def three_means_shift(self, frame: np.ndarray):
#         """
#         Apply three-means shift clustering to segment the fluid region.
#         """
#         if frame.shape[2] == 3:
#             gray_frame = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
#         else:
...
#         # Prepare data for mean shift - vectorized approach
...
#         data = valid_pixels.reshape(-1, 1).astype(np.float32)
#         _, labels, centers = cv.kmeans(data, 3, None, criteria, 5, cv.KMEANS_RANDOM_CENTERS)
        
#         # Create segmented image using vectorized operations
#         segmented = np.zeros_like(roi)
#         centers = centers.flatten().astype(np.uint8)
        
#         # Vectorized mapping
#         valid_mask = roi > 0
#         segmented[valid_mask] = centers[labels.flatten()]

#         # Get unique values and their counts in the segmented image
#         unique_values, counts = np.unique(segmented, return_counts=True)

#         # Filter out background (value 0) to see only the actual clusters
#         non_zero_mask = unique_values > 0
#         non_zero_values = unique_values[non_zero_mask]
#         non_zero_counts = counts[non_zero_mask]

#         return segmented, non_zero_values, non_zero_counts 

#     def loop(self, frame: np.ndarray):
#         """
#         Main loop to process the frame and find the edges of the fluid.
#         """
...
#         segmented_frame, values, counts = self.three_means_shift(frame_resized)
#         # self.last_five_whites.append(counts[-1])
#         # mean_counts = np.mean(self.last_five_whites)
#         self.xd_counts.append(counts[-1])

#         # cv.imshow("Segmented", segmented_frame)
        
#         # self.histogram(frame_resized)
        
#         # cv.line(frame_resized, (0, self.upper_line), (self.size[0], self.upper_line), (255, 0, 0), 2)

#         # cv.line(frame_resized, (0, self.lower_line), (self.size[0], self.lower_line), (255, 0, 0), 2)
...
#         measurement.loop(frame)
#         # Plot the xd_counts data
#         # if len(measurement.xd_counts) > 1:
#         #     counts = measurement.xd_counts

#         #     plt.clf()
#         #     plt.plot(counts, label='Counts of white pixels')
#         #     plt.xlabel('Frame Index')
#         #     plt.ylabel('Count')
#         #     plt.title('Counts of white pixels over time')

#         # plt.pause(0.01)

#         if measurement.frame_index % 100 == 0:
#             print('xd')
...
#     signal = np.array(measurement.xd_counts)
#     # save signal to a file
#     np.save('xd_counts.npy', signal)
    


    # detection
signal = np.load('xd_counts.npy')
